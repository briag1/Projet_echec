			...
			if (depart == null) {
				if (!echiquier.caselibre(panel.x, panel.y)) {
					if (echiquier.getJoueur(panel.x, panel.y) == doitJouer) {
						panel.setBackground(new Color( 249, 165, 155 ));
						depart = echiquier.getPiece(panel.x, panel.y);
						source = panel;
						vue.visualisezPossibilites(depart);
					} else {
						vue.afficherMessage("Erreur ! C'est à " + doitJouer.getNom() +" de jouer", "Mauvais joueur");
					}
				}
			} else {
				...
				if (panel == source) {
					...
					try {
						...
						if (estMat(trouverRoi(echiquier.getListePieces(), doitJouer))) {
							...
							vue.afficherMessage(doitJouer.getNom() + " est en échec !", "En échec !");
						} 
						...
						vue.afficherMessage(e.getMessage(), "Coup invalide !");
					} catch (Throwable e) {
